...
from models.Models.biformer import BiFormerBlock
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                Conv_BN_HSwish, MobileNetV3_InvertedResidual, CBRM, Shuffle_Block, stem, MBConvBlock,
                G_bneck, DepthSepConv, C3_DCN, C3STR, RepLKDeXt, BasicStage, PatchEmbed_FasterNet,
                PatchMerging_FasterNet, GSConv, VoVGSCSP, VoVGSCSPC}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3_DCN, C3STR, VoVGSCSP, VoVGSCSPC}:
        ...
        elif m in {CBAMBlock,GAM_Attention,SEAttention}:
            args = [ch[f], *args]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default=ROOT /'models/5-Wheat-experience-quedin/xiaoron/yolov5-fasterNet-biformer-Dcnv2-carafe-conv-kmeansup.yaml', help='model.yaml')

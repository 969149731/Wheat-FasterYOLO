...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'BENCHMARK': 'MOTCUSTOM',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'   写前面的那些会自己去下载数据集，我们验证自己的数据集时候可以自定义，如填写MOTCUSTOM
    ...
    def __init__(self, config=None):
        ...
        print("-----------------------------------------------------------------------------------------")
        print(self.class_list)     # ['pedestrian']
        print("-----------------------------------------------------------------------------------------")
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        """
        以MOT Challenge 2D框格式加载文件（gt或跟踪器）
        如果是is_gt，则返回包含以下字段的dict：
        [gt_ids，gt_classes]：1D ND阵列（每个det）的列表（每个时间步）。
        [gt_dets，gt_crowd_ignore_regions]：检测列表列表（针对每个时间步）。
        [gt_extras]：1D NDArray（每个det）的dict（每个额外）列表（每个时间步）。
        如果不是is_gt，则返回包含以下字段的dict：
        [tracker_ids，tracker_classes，tracker_confidences]：1D ND阵列（每个det）的列表（每个时间步）。
        [tracker_dets]：检测列表列表（针对每个时间步）。
        """

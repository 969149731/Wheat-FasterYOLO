...
class HOTA(_BaseMetric):
    ...
    def __init__(self, config=None):
        ...
        self.float_array_fields = ['HOTA', 'DetA', 'AssA', 'DetRe', 'AssRe','DetPr',  'AssPr', 'LocA', 'OWTA']
        #定义自己要画图的画出来的指标   下下面的代码和上面的指标出现顺序要一样，可以在上面调顺序
        self.myfloat_array_fields = ['HOTA', 'DetA', 'AssA', 'DetRe', 'AssRe']
    ...
    def plot_single_tracker_results(self, table_res, tracker, cls, output_folder):
        ...
        #  下面的是该改的  画图出来的颜色  上面原来的  styles_to_plot = ['r', 'b', 'g', 'b--', 'b:', 'g--', 'g:', 'm']
        styles_to_plot = ['r', 'b', 'g','orange','#8B00FF']
        # 下面的是改过的 for name, style in zip(self.float_array_fields, styles_to_plot):
        for name, style in zip(self.myfloat_array_fields, styles_to_plot):
            plt.plot(self.array_labels, res[name], style)
        plt.xlabel('alpha')
        plt.ylabel('score')
        # plt.title(tracker + ' - ' + cls)        # 这里标记一下，改画的图里面的标题名字\
        plt.title('Xumai 45')
        plt.axis([0, 1, 0, 1])
        legend = []
        ...
        out_file = os.path.join(output_folder,'result_plot.pdf')    # 这里标记一下，改exp里面的pdf和png的文件名的
        os.makedirs(os.path.dirname(out_file), exist_ok=True)
        plt.savefig(out_file)
        plt.savefig(out_file.replace('.pdf', '.jpg'),dpi=500)
        plt.clf()
